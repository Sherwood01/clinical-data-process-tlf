...
echo "     gcloud sql instances create tlf-postgres --tier=db-f1-micro --region=$REGION"
echo ""
echo "  2. Set up Secret Manager secrets:"
echo "     gcloud secrets create tlf-database-url --replication-policy=automatic"
echo ""
echo "  3. Run database migrations:"
echo "     gcloud run jobs execute tlf-migrate"
echo ""
echo "  4. Register Upstash Redis (free tier) and set CELERY_BROKER_URL"
echo ""
echo "  5. Deploy Stack Auth separately:"
echo "     ./cloudbuild/deploy-stack-auth.sh $PROJECT_ID"
echo ""
echo "  6. Configure custom domain or use the Cloud Run URLs above"
